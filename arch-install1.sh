#!/bin/bash
set -euo pipefail

##############################################################################
# Arch Linux Installation Script - HP Pavilion Gaming
# No Encryption | Single-GPU Passthrough Ready | Hyprland
# User: muhammad | Hostname: arch | Timezone: Africa/Cairo
##############################################################################

# Color codes
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m'

# Configuration
USERNAME="muhammad"
HOSTNAME="arch"
TIMEZONE="Africa/Cairo"
LOCALE="en_US.UTF-8"
DISK="/dev/nvme0n1"

##############################################################################
# Helper Functions
##############################################################################

print_msg() {
    echo -e "${GREEN}[*]${NC} $1"
}

print_error() {
    echo -e "${RED}[ERROR]${NC} $1"
    exit 1
}

print_warning() {
    echo -e "${YELLOW}[WARNING]${NC} $1"
}

print_step() {
    echo -e "${BLUE}[STEP]${NC} $1"
}

confirm_continue() {
    echo -e "${YELLOW}$1${NC}"
    read -p "Press ENTER to continue or CTRL+C to abort: "
}

##############################################################################
# Pre-flight Checks
##############################################################################

print_step "Pre-flight checks"

# Check if running as root
if [[ $EUID -ne 0 ]]; then
   print_error "This script must be run as root"
fi

# Check if UEFI mode
if [[ ! -d /sys/firmware/efi/efivars ]]; then
    print_error "Not booted in UEFI mode. Please boot in UEFI mode."
fi

# Check internet connectivity
print_msg "Checking internet connectivity..."
if ! ping -c 1 archlinux.org &>/dev/null; then
    print_error "No internet connection. Please connect to the internet first.
    
To connect WiFi:
  iwctl
  station wlan0 connect \"YOUR_WIFI_NAME\"
  exit
  
Then re-run this script."
fi

# Verify disk exists
if [[ ! -b ${DISK} ]]; then
    print_error "Disk ${DISK} not found. Available disks:"
    lsblk -d -o NAME,SIZE,TYPE | grep disk
fi

# Display disk info
print_msg "Target disk: ${DISK}"
lsblk ${DISK}
confirm_continue "WARNING: This will DESTROY ALL DATA on ${DISK}!"

##############################################################################
# Cleanup any previous attempts
##############################################################################

print_step "Cleaning up any previous installation attempts"

# Unmount everything under /mnt
umount -R /mnt 2>/dev/null || true
swapoff -a 2>/dev/null || true

# Wait for unmounts to complete
sleep 2

##############################################################################
# Phase 1: Disk Partitioning
##############################################################################

print_step "Phase 1: Partitioning disk"

# Wipe existing partition table
wipefs -af ${DISK}
sgdisk --zap-all ${DISK}

# Create new GPT partition table and partitions
sgdisk -n 1:0:+1G -t 1:ef00 -c 1:"EFI" ${DISK}
sgdisk -n 2:0:+1G -t 2:8300 -c 2:"BOOT" ${DISK}
sgdisk -n 3:0:0 -t 3:8300 -c 3:"ROOT" ${DISK}

# Inform kernel of changes
partprobe ${DISK}
sleep 3

print_msg "Partition layout:"
lsblk ${DISK}

##############################################################################
# Phase 2: Format Filesystems
##############################################################################

print_step "Phase 2: Formatting filesystems"

# Format partitions
print_msg "Formatting EFI partition..."
mkfs.fat -F32 ${DISK}p1

print_msg "Formatting boot partition..."
mkfs.ext4 -F ${DISK}p2

print_msg "Formatting root partition with BTRFS..."
mkfs.btrfs -f -L archlinux ${DISK}p3

##############################################################################
# Phase 3: BTRFS Subvolumes
##############################################################################

print_step "Phase 3: Creating BTRFS subvolumes"

# Mount root partition temporarily
mount ${DISK}p3 /mnt

# Create subvolumes
print_msg "Creating subvolumes..."
btrfs subvolume create /mnt/@
btrfs subvolume create /mnt/@home
btrfs subvolume create /mnt/@var_log
btrfs subvolume create /mnt/@var_lib_docker
btrfs subvolume create /mnt/@snapshots
btrfs subvolume create /mnt/@qemu
btrfs subvolume create /mnt/@shared
btrfs subvolume create /mnt/@ai_workspace

# List created subvolumes
print_msg "Subvolumes created:"
btrfs subvolume list /mnt

# Unmount
umount /mnt

##############################################################################
# Phase 4: Mount with Optimized Options
##############################################################################

print_step "Phase 4: Mounting filesystems with optimized options"

# Mount root subvolume
print_msg "Mounting @ (root)..."
mount -o noatime,compress=zstd:1,space_cache=v2,ssd,subvol=@ ${DISK}p3 /mnt

# Create mount point directories
mkdir -p /mnt/{boot,boot/efi,home,var/log,var/lib/docker,.snapshots,var/lib/libvirt/images,mnt/shared,home/${USERNAME}/ai_workspace}

# Mount other subvolumes
print_msg "Mounting subvolumes..."
mount -o noatime,compress=zstd:1,space_cache=v2,ssd,subvol=@home ${DISK}p3 /mnt/home
mount -o noatime,compress=zstd:2,space_cache=v2,ssd,subvol=@var_log ${DISK}p3 /mnt/var/log
mount -o noatime,compress=no,space_cache=v2,ssd,nodatacow,subvol=@var_lib_docker ${DISK}p3 /mnt/var/lib/docker
mount -o noatime,compress=zstd:1,space_cache=v2,ssd,subvol=@snapshots ${DISK}p3 /mnt/.snapshots
mount -o noatime,compress=no,space_cache=v2,ssd,nodatacow,subvol=@qemu ${DISK}p3 /mnt/var/lib/libvirt/images
mount -o noatime,compress=zstd:3,space_cache=v2,ssd,subvol=@shared ${DISK}p3 /mnt/mnt/shared
mount -o noatime,compress=no,space_cache=v2,ssd,nodatacow,subvol=@ai_workspace ${DISK}p3 /mnt/home/${USERNAME}/ai_workspace

# Mount boot partitions
print_msg "Mounting boot partitions..."
mount ${DISK}p2 /mnt/boot
mount ${DISK}p1 /mnt/boot/efi

# Verify mounts
print_msg "Current mount structure:"
lsblk -f ${DISK}

##############################################################################
# Phase 5: Configure Pacman and Mirrorlist
##############################################################################

print_step "Phase 5: Configuring package manager"

# Update mirrorlist for faster downloads
print_msg "Updating mirrorlist..."
reflector --country Egypt,Germany --age 12 --protocol https --sort rate --save /etc/pacman.d/mirrorlist 2>/dev/null || print_warning "Reflector failed, using default mirrorlist"

# Enable parallel downloads
sed -i 's/^#ParallelDownloads/ParallelDownloads/' /etc/pacman.conf

# Add CachyOS repository
print_msg "Adding CachyOS repository..."
if ! grep -q "\[cachyos\]" /etc/pacman.conf; then
    cat >> /etc/pacman.conf << 'EOF'

# CachyOS Repository
[cachyos]
Include = /etc/pacman.d/cachyos-mirrorlist
EOF

    # Create cachyos mirrorlist
    cat > /etc/pacman.d/cachyos-mirrorlist << 'EOF'
Server = https://mirror.cachyos.org/repo/$arch/$repo
Server = https://cdn.cachyos.org/repo/$arch/$repo
EOF
fi

# Import CachyOS keys
print_msg "Importing CachyOS GPG keys..."
pacman-key --recv-keys F3B607488DB35A47 --keyserver keyserver.ubuntu.com 2>/dev/null || true
pacman-key --lsign-key F3B607488DB35A47 2>/dev/null || true

# Update package databases
print_msg "Updating package databases..."
pacman -Sy --noconfirm

##############################################################################
# Phase 6: Install Base System
##############################################################################

print_step "Phase 6: Installing base system (this takes 10-15 minutes)"

print_msg "Installing packages via pacstrap..."

pacstrap -K /mnt \
    base linux-cachyos linux-cachyos-headers \
    linux-firmware amd-ucode \
    base-devel git neovim vim nano \
    btrfs-progs \
    networkmanager openssh \
    nvidia-dkms nvidia-utils lib32-nvidia-utils \
    qemu-full libvirt virt-manager virt-viewer ovmf swtpm edk2-ovmf \
    dnsmasq iptables-nft bridge-utils \
    usbutils libusb \
    docker docker-compose docker-buildx \
    python python-pip python-virtualenv python-numpy \
    ffmpeg \
    hyprland xdg-desktop-portal-hyprland \
    polkit-kde-agent qt5-wayland qt6-wayland \
    sddm \
    waybar rofi-wayland dunst swww grim slurp wl-clipboard cliphist \
    kitty \
    thunar thunar-volman gvfs \
    pipewire pipewire-pulse pipewire-alsa pipewire-jack wireplumber pavucontrol \
    ttf-dejavu ttf-liberation noto-fonts noto-fonts-emoji \
    brightnessctl playerctl bluez bluez-utils \
    nm-connection-editor \
    zram-generator \
    man-db man-pages \
    htop btop \
    wget curl \
    unzip zip \
    || print_error "Pacstrap failed. Check your internet connection."

print_msg "Base system installed successfully"

##############################################################################
# Phase 7: Generate fstab
##############################################################################

print_step "Phase 7: Generating fstab"

genfstab -U /mnt >> /mnt/etc/fstab

print_msg "Generated fstab:"
cat /mnt/etc/fstab

##############################################################################
# Phase 8: Chroot and Configure System
##############################################################################

print_step "Phase 8: Configuring system in chroot"

# Create chroot configuration script
cat > /mnt/root/chroot-setup.sh << 'CHROOTEOF'
#!/bin/bash
set -euo pipefail

USERNAME="muhammad"
HOSTNAME="arch"
TIMEZONE="Africa/Cairo"
LOCALE="en_US.UTF-8"
DISK="/dev/nvme0n1"

echo "[*] Setting timezone..."
ln -sf /usr/share/zoneinfo/${TIMEZONE} /etc/localtime
hwclock --systohc

echo "[*] Configuring locale..."
sed -i "s/^#${LOCALE}/${LOCALE}/" /etc/locale.gen
locale-gen
echo "LANG=${LOCALE}" > /etc/locale.conf

echo "[*] Setting hostname..."
echo "${HOSTNAME}" > /etc/hostname

echo "[*] Configuring hosts file..."
cat > /etc/hosts << EOF
127.0.0.1   localhost
::1         localhost
127.0.1.1   ${HOSTNAME}.localdomain ${HOSTNAME}
EOF

echo "[*] Configuring mkinitcpio..."
cat > /etc/mkinitcpio.conf << 'EOF'
MODULES=(amdgpu nvidia nvidia_modeset nvidia_uvm nvidia_drm)
BINARIES=()
FILES=()
HOOKS=(base systemd autodetect microcode modconf kms keyboard sd-vconsole block filesystems fsck)
EOF

mkinitcpio -P

echo "[*] Configuring NVIDIA..."
mkdir -p /etc/modprobe.d

cat > /etc/modprobe.d/nvidia.conf << 'EOF'
options nvidia NVreg_PreserveVideoMemoryAllocations=1
options nvidia_drm modeset=1
EOF

cat > /etc/modprobe.d/vfio.conf << 'EOF'
options vfio-pci ids=10de:25a0,10de:2291
softdep nvidia pre: vfio-pci
EOF

cat > /etc/modprobe.d/kvm.conf << 'EOF'
options kvm_amd nested=1
options kvm_amd npt=1
options kvm_amd avic=1
EOF

echo "[*] Installing bootloader..."
bootctl install

cat > /boot/loader/loader.conf << 'EOF'
default arch.conf
timeout 5
console-mode max
editor no
EOF

ROOT_UUID=$(blkid -s UUID -o value ${DISK}p3)

cat > /boot/loader/entries/arch.conf << EOF
title   Arch Linux
linux   /vmlinuz-linux-cachyos
initrd  /amd-ucode.img
initrd  /initramfs-linux-cachyos.img
options root=UUID=${ROOT_UUID} rootflags=subvol=@ rw amd_iommu=on iommu=pt video=efifb:off kvm.ignore_msrs=1 kvm_amd.npt=1 kvm_amd.avic=1
EOF

cat > /boot/loader/entries/arch-fallback.conf << EOF
title   Arch Linux (Fallback)
linux   /vmlinuz-linux-cachyos
initrd  /amd-ucode.img
initrd  /initramfs-linux-cachyos-fallback.img
options root=UUID=${ROOT_UUID} rootflags=subvol=@ rw
EOF

echo "[*] Setting root password..."
echo "Enter password for root:"
passwd

echo "[*] Creating user: ${USERNAME}..."
useradd -m -G wheel,libvirt,docker,video,audio,input -s /bin/bash ${USERNAME}
echo "Enter password for ${USERNAME}:"
passwd ${USERNAME}

echo "[*] Configuring sudo..."
echo "%wheel ALL=(ALL:ALL) ALL" > /etc/sudoers.d/wheel
chmod 440 /etc/sudoers.d/wheel

echo "[*] Enabling services..."
systemctl enable NetworkManager
systemctl enable sddm
systemctl enable libvirtd
systemctl enable docker
systemctl enable bluetooth

echo "[*] Configuring libvirt..."
sed -i 's/^#unix_sock_group = "libvirt"/unix_sock_group = "libvirt"/' /etc/libvirt/libvirtd.conf
sed -i 's/^#unix_sock_rw_perms = "0770"/unix_sock_rw_perms = "0770"/' /etc/libvirt/libvirtd.conf

cat >> /etc/libvirt/qemu.conf << EOF
user = "${USERNAME}"
group = "libvirt"
EOF

echo "[*] Configuring Docker..."
mkdir -p /etc/docker
cat > /etc/docker/daemon.json << 'EOF'
{
    "data-root": "/var/lib/docker",
    "storage-driver": "overlay2"
}
EOF

echo "[*] Configuring zram..."
cat > /etc/systemd/zram-generator.conf << 'EOF'
[zram0]
zram-size = ram / 4
compression-algorithm = zstd
swap-priority = 100
EOF

echo "[*] Creating swap file..."
mkdir -p /swap
truncate -s 0 /swap/swapfile
chattr +C /swap/swapfile
dd if=/dev/zero of=/swap/swapfile bs=1M count=16384 status=progress
chmod 600 /swap/swapfile
mkswap /swap/swapfile

echo "/swap/swapfile none swap sw,pri=10 0 0" >> /etc/fstab

echo "[*] Disabling CoW on performance-critical subvolumes..."
chattr +C /var/lib/docker || true
chattr +C /var/lib/libvirt/images || true
chattr +C /home/${USERNAME}/ai_workspace || true

echo "[*] Creating AI workspace structure..."
mkdir -p /home/${USERNAME}/ai_workspace/{videos_input,videos_processing,models/{whisper,deepface,ocr},output/{transcripts,facial_analysis,ocr_results},cache}
chown -R ${USERNAME}:${USERNAME} /home/${USERNAME}/ai_workspace
chmod -R 755 /home/${USERNAME}/ai_workspace

echo "[*] Creating Hyprland config directory..."
mkdir -p /home/${USERNAME}/.config/hypr
chown -R ${USERNAME}:${USERNAME} /home/${USERNAME}/.config

echo "[*] Creating post-install instructions..."
cat > /home/${USERNAME}/POST_INSTALL.txt << 'POSTEOF'